...
JoinLoop::JoinLoop(const JoinLoopKind kind,
                   ...
                   const std::function<llvm::Value*(const std::vector<llvm::Value*>&)>& is_deleted,
    ...
    switch (join_loop.kind_) {
      ...
      case JoinLoopKind::Set: {
        ...
        if (join_loop.is_deleted_) {
          ...
          const auto row_is_deleted = join_loop.is_deleted_(iterators);
      ...
      case JoinLoopKind::Singleton: {
        ...
        if (join_loop.is_deleted_) {
          match_found = builder.CreateAnd(match_found, builder.CreateNot(join_loop.is_deleted_(iterators)));
        }
